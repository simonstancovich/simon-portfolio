...
export default async function Hero() {
  const profile = await sanityFetch<Profile>({
    query: PROFILE_QUERY,
    tags: ["profile"],
    revalidate: 60,
  });
            ...
            <div className="mt-6 flex flex-wrap items-center gap-3">
              ...
              {github && (
                <Link
                  href={github}
                  target="_blank"
                  className="inline-flex h-10 items-center justify-center rounded-lg px-4 text-sm font-medium underline underline-offset-4 hover:opacity-90"
                >
                  GitHub
                </Link>
              )}
